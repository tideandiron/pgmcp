    ...
    pub(crate) fn from_str(s: &str) -> Result<Self, McpError> {
        match s {
            ...
            other => Err(McpError::param_invalid(
                "format",
                format!(
                    "unknown format '{other}'; must be one of: json, json_compact, csv"
                ),
            )),
        }
    }
...
impl QueryResponse {
    ...
    pub(crate) fn to_json_string(&self) -> String {
        ...
        let rows_value = match self.format {
            ...
            OutputFormat::Csv => {
                // CSV is returned as a raw string.
...
mod tests {
    ...
    fn output_format_as_str_round_trips() {
        for (input, expected) in [("json", "json"), ("json_compact", "json_compact"), ("csv", "csv")] {
    ...
    fn query_response_json_format_embeds_rows_as_array() {
        ...
        let s = resp.to_json_string();
        let parsed: serde_json::Value = serde_json::from_str(&s).expect("must be valid JSON");
        assert!(parsed["rows"].is_array(), "JSON format rows should be an array");
    ...
    fn query_response_csv_format_returns_string() {
        ...
        let s = resp.to_json_string();
        let parsed: serde_json::Value = serde_json::from_str(&s).expect("must be valid JSON");
        assert!(parsed["rows"].is_string(), "CSV format rows should be a string");
